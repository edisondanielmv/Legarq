...
if (!content.includes(tSrc)) {
  console.log("tSrc not found");
} else if (!content.includes(rowEndSrc)) {
  console.log("rowEndSrc not found");
} else {
  content = content.replace(tSrc, tDst);
  content = content.replace(rowEndSrc, rowEndDst);
  fs.writeFileSync('src/pages/dashboard/FinancialReports.tsx', content);
  console.log("Updated FinancialReports.tsx correctly.");
}
