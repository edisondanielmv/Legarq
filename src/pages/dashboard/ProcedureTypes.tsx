...
export default function ProcedureTypes() {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto relative font-sans selection:bg-[#E3000F] selection:text-white">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-black text-gray-900 tracking-tight">Tipos de Trámite</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {types.length === 0 ? (
          <div className="col-span-full py-16 text-center bg-white rounded-[24px] border-2 border-dashed border-gray-100">
            <div className="bg-gray-50 w-16 h-16 rounded-[20px] flex items-center justify-center mx-auto mb-4">
              <Briefcase className="w-8 h-8 text-gray-200" />
            </div>
            <h3 className="text-lg font-black text-gray-900 mb-1">Sin registros</h3>
            <p className="text-gray-400 max-w-xs mx-auto text-[9px] font-black uppercase tracking-widest leading-relaxed">No hay tipos de trámite registrados aún.</p>
          </div>
        ) : (
          types.map((type) => (
            <div key={type.id} className="bg-white p-5 rounded-[24px] border border-gray-100 shadow-sm hover:shadow-md transition-all group">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center border border-gray-100 group-hover:scale-110 transition-transform">
                    <Briefcase className="w-5 h-5 text-gray-400 group-hover:text-[#E3000F] transition-colors" />
                  </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-1.5 text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">
                  <ListChecks className="w-3 h-3 text-[#E3000F]" /> Pasos del Proceso
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {getStepsList(type.steps).length === 0 ? (
                    <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest italic">Sin pasos definidos</span>
                  ) : (
                    getStepsList(type.steps).slice(0, 4).map((step, i) => (
                      <span key={i} className="px-2 py-1 bg-gray-50 text-gray-600 text-[8px] font-black uppercase tracking-widest rounded-lg border border-gray-100 truncate max-w-[120px]">
                        {step}
                      </span>
                    ))
                  )}
                  {getStepsList(type.steps).length > 4 && (
                    <span className="px-2 py-1 bg-[#1A1A1A] text-white text-[8px] font-black uppercase tracking-widest rounded-lg">
