/**
 * @fileoverview Backend para Gestión de Trámites Legar - VERSION INTEGRAL
...
function deleteProcedure(data) {
  ...
  var rows = sheet.getDataRange().getValues();
  var h = rows[0];
  var idToDelete = idFromData(data);
  
  if (!idToDelete) throw new Error('ID de trámite no proporcionado');

  for (var i = 1; i < rows.length; i++) {
    // Check by real ID (index 0) or by Code (index 1)
    if (String(rows[i][0] || '') === idToDelete || String(rows[i][1] || '') === idToDelete) {
      var realId = String(rows[i][0] || ''); // Always use the UUID for relational deletions
      var procData = {};
      ...
      deleteRowsByColumn('Bitacora', 1, realId);
      deleteRowsByColumn('Archivos', 1, realId);
      sheet.deleteRow(i + 1);

      if (procData.clientUsername) {
        try {
          var clientUser = String(procData.clientUsername || '').trim();
          if (clientUser) {
            var allProcs = getSheetData('Tramites');
            var otherProcs = allProcs.filter(function(p) {
              return String(p.clientUsername || '').trim() === clientUser;
            });
            if (otherProcs.length === 0) deleteUser({ username: clientUser });
          }
        } catch (e) {}
      }
      return { success: true };
  ...
  throw new Error('Trámite no encontrado con ID/Código: ' + idToDelete);
}
